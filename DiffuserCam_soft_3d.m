function [varargout] =  DiffuserCam_soft_3d(v,h,d,tau,iso,varargin)
...
if size(v,1) ~= 0  %If no v, h, or d passed in, skip gradient thresholding
    if iso == true
        ...
        varargout{1} = DiffuserCam_soft(v, tau);
        varargout{2} = DiffuserCam_soft(h, tau);
        varargout{3} = DiffuserCam_soft(d, tau);
    end
else  %Do native soft thresholding
